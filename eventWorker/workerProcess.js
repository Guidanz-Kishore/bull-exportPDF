...
module.exports=processQueue=(job)=>new Promise((resolve,reject)=>{
  console.log(`${job.data.fileName} is going to sleep with pid `+process.pid);
    ...
    if (!fs.existsSync(folderPath+'/.'+job.data.downloadPath)) {
      fs.mkdirSync(folderPath+'/.'+job.data.downloadPath);
     }
     let path=`${folderPath}/${job.data.downloadPath}/${job.data.fileName}.${job.data.downloadType}`;
